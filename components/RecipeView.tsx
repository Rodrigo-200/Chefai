...
const DesktopIngredientRow: React.FC<{ ingredient: { name: string; amount: string; unit: string; notes?: string } }> = ({ ingredient }) => {
    const [checked, setChecked] = useState(false);
    return (
        <div 
            onClick={() => setChecked(!checked)}
            className={`flex items-start gap-3 py-3 border-b border-gray-100 dark:border-gray-800 cursor-pointer group transition-all ${checked ? 'opacity-50' : ''}`}
        >
            <div className={`mt-1 h-5 w-5 rounded-full border-2 flex items-center justify-center transition-colors ${checked ? 'bg-chef-600 border-chef-600' : 'border-gray-300 dark:border-gray-600 group-hover:border-chef-400'}`}>
                {checked && <Check size={12} className="text-white" />}
            </div>
            <div className="flex-1">
                <p className={`text-base ${checked ? 'text-gray-500 line-through' : 'text-gray-900 dark:text-white'}`}>
                    <span className="font-bold">{ingredient.amount} {ingredient.unit}</span> {ingredient.name}
                </p>
                {ingredient.notes && <p className="text-xs text-gray-400 mt-0.5 italic">{ingredient.notes}</p>}
            </div>
        </div>
    );
};
...
export const RecipeView: React.FC<RecipeViewProps> = ({ recipe, onBack, onSave, onUnsave, onDelete, onUpdate, isSaved, isLoggedIn, folders = [] }) => {
  ...
  const currentFolderId = editedRecipe.folderId || recipe.folderId;
  const currentFolder = folders.find(f => f.id === currentFolderId);

  const handleFolderQuickChange = (folderId: string) => {
    if (isEditing) {
      updateField('folderId', folderId || undefined);
      return;
    }
    const updated = { ...recipe, folderId: folderId || undefined };
  ...
  return (
    <div className="bg-white dark:bg-gray-900 min-h-screen pb-20 animate-in slide-in-from-bottom-4 duration-300">
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 pt-6">
        <div className="relative h-[42vh] sm:h-[50vh] lg:h-[45vh] w-full rounded-3xl overflow-hidden bg-gray-100 dark:bg-gray-800 shadow-xl shadow-gray-200/60 dark:shadow-none">
      ...
      {/* Main Content */}
      <div className="max-w-5xl mx-auto px-4 sm:px-6 -mt-10 lg:-mt-14 relative z-10 space-y-10">
        <div className="space-y-10">
          {/* Title Card */}
          <div className="bg-white dark:bg-gray-900 rounded-3xl p-8 shadow-xl shadow-gray-200/50 dark:shadow-none border border-gray-100 dark:border-gray-800 text-center">
              {isEditing ? (
                  ...
                  <>
                      ...
                      <p className="text-lg md:text-xl text-gray-600 dark:text-gray-300 leading-relaxed max-w-2xl mx-auto mb-6 font-serif">
                          {recipe.description}
                      </p>

                      {/* Collection Selector - Custom Dropdown */}
                      <div className="flex items-center justify-center mb-8 relative z-20" ref={collectionDropdownRef}>
                          <button
                              onClick={() => setCollectionMenuOpen(!collectionMenuOpen)}
                              className={`relative inline-flex items-center gap-2 px-5 py-2.5 rounded-full border transition-all duration-200 ${
                                  currentFolder 
                                  ? 'bg-chef-50 dark:bg-chef-900/20 border-chef-200 dark:border-chef-800 text-chef-700 dark:text-chef-300 hover:border-chef-400' 
                                  : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 hover:border-gray-400 dark:hover:border-gray-500 hover:shadow-sm'
                              }`}
                          >
                              {currentFolder ? (
                                  <>
                                      <FolderIcon size={16} fill="currentColor" className="opacity-50" />
                                      <span className="font-medium text-sm">Saved in <strong>{currentFolder.name}</strong></span>
                                      <ChevronDown size={14} className={`ml-1 transition-transform duration-200 ${collectionMenuOpen ? 'rotate-180' : ''}`} />
                                  </>
                              ) : (
                                  <>
                                      <Plus size={16} />
                          ...
                          {/* Dropdown Menu */}
                          {collectionMenuOpen && (
                              <div className="absolute top-full mt-2 w-64 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden animate-in fade-in zoom-in-95 duration-100 text-left">
                                  <div className="p-2">
                                      <div className="text-xs font-bold text-gray-400 uppercase tracking-wider px-3 py-2">Select Collection</div>
                                      {folders.length > 0 ? (
                                          <div className="max-h-60 overflow-y-auto">
                                              <button
                                                  onClick={() => { handleFolderQuickChange(''); setCollectionMenuOpen(false); }}
                                                  className={`w-full text-left px-3 py-2 rounded-lg text-sm flex items-center gap-2 transition-colors ${!currentFolderId ? 'bg-chef-50 dark:bg-chef-900/30 text-chef-700 dark:text-chef-300' : 'text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700'}`}
                                              >
                                                  <X size={14} />
                                                  None (Remove)
                                              </button>
                                              {folders.map(f => (
                                                  <button
                                                      key={f.id}
                                                      onClick={() => { handleFolderQuickChange(f.id); setCollectionMenuOpen(false); }}
                                                      className={`w-full text-left px-3 py-2 rounded-lg text-sm flex items-center gap-2 transition-colors ${currentFolderId === f.id ? 'bg-chef-50 dark:bg-chef-900/30 text-chef-700 dark:text-chef-300 font-medium' : 'text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700'}`}
                                                  >
                                                      <FolderIcon size={14} fill={currentFolderId === f.id ? "currentColor" : "none"} />
                                                      {f.name}
                                                      {currentFolderId === f.id && <Check size={14} className="ml-auto" />}
                                                  </button>
                                              ))}
                                          </div>
                                      ) : (
          ...
          <div className="grid md:grid-cols-[1fr_1.5fr] gap-12 pb-6 lg:pb-10">
              {/* Ingredients */}
              <div>
                  ...
                  {isEditing ? (
                      ...
                      <>
                        ...
                        {/* Desktop View (Clean List) */}
                        <div className="hidden md:grid md:grid-cols-2 gap-x-12 gap-y-2">
                            {recipe.ingredients.map((ingredient, idx) => (
                                <DesktopIngredientRow key={idx} ingredient={ingredient} />
                            ))}
                        </div>
                      </>
                  )}
